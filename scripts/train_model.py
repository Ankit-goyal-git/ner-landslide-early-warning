...
def train_and_evaluate(features_csv_path, models_dir, reports_dir):
    ...
    # Train: <= 2013, Val: 2014 - 2015, Test: >= 2016
    train_mask = df['year'] <= 2013
    val_mask = (df['year'] >= 2014) & (df['year'] <= 2015)
    ...
    print(f"  Train set (<= 2013): {X_train.shape[0]} samples (Pos: {y_train.sum()}, Neg: {(y_train == 0).sum()})")
    print(f"  Val set (2014-2015): {X_val.shape[0]} samples (Pos: {y_val.sum()}, Neg: {(y_val == 0).sum()})")
    print(f"  Test set (>= 2016):  {X_test.shape[0]} samples (Pos: {y_test.sum()}, Neg: {(y_test == 0).sum()})")
    ...
    for name, pipeline in models.items():
        ...
        print(f"[{name}] Test Metrics -> Precision: {test_prec:.4f}, Recall: {test_rec:.4f}, F1: {test_f1:.4f}, ROC-AUC: {test_auc:.4f}, PR-AUC: {test_pr_auc:.4f}")
    ...
    metadata = {
        ...
        "split_strategy": "Temporal (Train <= 2013, Val 2014-2015, Test >= 2016)",
...
- **Split Strategy**: Strictly Temporal Split to prevent spatial-temporal leakage.
  - **Training Set (<= 2013)**: {len(X_train)} samples ({y_train.sum()} positive events, {(y_train == 0).sum()} pseudo-absences)
  - **Validation Set (2014–2015)**: {len(X_val)} samples ({y_val.sum()} positive events, {(y_val == 0).sum()} pseudo-absences)
  - **Test Set (2016 Hold-out)**: {len(X_test)} samples ({y_test.sum()} positive events, {(y_test == 0).sum()} pseudo-absences)
...
"""
    for m_name, res in results.items():
        t = res['test']
        report_content += f"| **{m_name.replace('_', ' ')}** | {t['precision']:.4f} | {t['recall']:.4f} | {t['f1']:.4f} | {t['roc_auc']:.4f} | {t['pr_auc']:.4f} |\n"
